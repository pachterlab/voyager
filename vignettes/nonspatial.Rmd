...
Download dataset: https://support.10xgenomics.com/single-cell-gene-expression/datasets/3.0.2/5k_pbmc_v3_nextgem? This dataset has already been filtered.
```{r}
...
Next, it is customary to make a UMAP plot based on the PCA project. We do not recommend producing a UMAP since the procedure distorts distances, and does not respect either local or global structure in the data (Chari, Banerjee, and Pachter 2021). However, for completeness, we show how to produce a UMAP below:
```{r}
plotUMAP(sce, color_by = "cluster")
```

Instead of a heuristic UMAP examination, we apply a spatially motivated analysis to the k-nearest neighbor graph in the PCA projection, namely Moran's I:
```{r}
...
It's just so concentrated around the origin. But for the cells that do express this gene, there are interesting clusters.
...
That's pretty strong. Next we came examine local correlations: 
