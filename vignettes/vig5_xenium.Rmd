...
That's should be in pixels. There's a very long tail. The nuclei are so much smaller than the cells.
...
Here the nuclei segmentations are plotted instead of cell segmentation. The nuclei are so much smaller that they're kind of hard to see.
```{r}
plotSpatialFeature(sfe, "nucleus_area", colGeometryName = "nucSeg")
```

There's an outlier near the right edge of the section, throwing off the dynamic range of this plot. Upon inspection of the H&E image, that outlier is a bit of tissue debris that doesn't look like a cell. But we can still that cells in the dense, gland like regions tend to have larger nuclei. This may be biological, or that nuclei are so densely packed in those regions that they are more likely to be undersegmented, i.e. when multiple nuclei are counted as one by the nuclei segmentation program, or both.
...
This is all over the place but that outlier is obvious. There are more cells with both small nuclei and low proportion of area occupied by the nucleus.
...
The histogram is dominated by the bin at 0 and there are some extreme outliers too few to be seen but evident from the scale of the x axis. We also plot the histogram only for cells with at least 1 count from a negative control. The NA's come from cells that got segmented but have no transcripts detected.
```{r}
...
The counts are low, mostly 0, but there are outliers with up to 10 counts of all types aggregated. Then the outlier with 50% of counts from negative controls must have very low total real transcript counts to begin with.
...
They do seem to be smaller. Outliers for negative probe control and negative codeword control are also hard to see on the plot, so their plots are skipped here. But the top left region in the tissue tends to have more counts from antisense controls. 
```{r}
...
There are at most 3 count per cell per type. For the non-outliers, each type is at most around 1%, so this data looks like really good. 
...
Get the significant marker for each cluster to plot
```{r}
