...
plotGeometry(sfe, "cellSeg", fill = FALSE, dark = TRUE,
             image_id = "morphology_focus", 
             channel = 3:1, bbox = bbox3) +
    geom_sf(data = nuc, fill = NA, color = "#85C1E9")
```
...
plotColDataHistogram(sfe, cols_use, bins = 20) +
...
The counts are low, mostly zero, but there are outliers with up to 2 counts of all types aggregated. Then the outlier with 30% of counts from negative controls must have very low total real transcript counts to begin with.

```{r}
names(colData(sfe))
```

Where are the cells with any negative control detected in space?
```{r}
plotSpatialFeature(sfe, "subsets_any_neg_sum", colGeometryName = "cellSeg") +
    theme(legend.position = "bottom")
```

They appear to be randomly distributed in space. No spatial pattern is apparent. Unlike in the first preview dataset (`JanesickBreastData()`), 
