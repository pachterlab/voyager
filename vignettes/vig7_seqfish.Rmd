...
download.file("https://caltech.box.com/s/ulrqr1gk7oh21h9ejgua1xajs6dsbvrj",'./seqfish_em3.Rds')
...
cells_fov <- ggplot(data.frame(colData(sfe)), aes(pos,)) +
  geom_bar() + 
  theme_minimal() + 
  labs(
    x = "",
...
meta <- meta %>% 
  ...
  mutate(prop_lq = nLQ_FOV/nCells_FOV) %>% distinct(pos, prop_lq)
...
```{r}
pcs <- reducedDim(sfe, "PCA")[,1:2]
ord <- rownames(colData(sfe))
pcs <- pcs[ord,]

colData(sfe)$pc1  <- pcs[,1]
colData(sfe)$pc2  <- pcs[,2]

plotSpatialFeature(sfe, c("pc1","pc2"), colGeometryName = "seg_coords") +
    theme_void()
...
```{r}
plotSpatialFeature(sfe, c("cluster", "celltype_mapped_refined"), colGeometryName = "seg_coords") + guides(colour = guide_legend(override.aes = list(size=2)))
```
