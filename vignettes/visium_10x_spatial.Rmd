...
We can use the package [gget](https://pachterlab.github.io/gget/) to get additional information on these genes:
```{r}
gget_info <- gget$info(top_moran)
select(gget_info, primary_gene_name, ncbi_description)
```
...
select(gget_info_neg, primary_gene_name, ncbi_description)
```
...
select(gget_info2, primary_gene_name, ncbi_description)
```
