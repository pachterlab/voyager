...
# Note: this can take up to 30 minutes (many of the dependencies have C++ code, which needs to be compiled)
# Google Colab provides a mostly fresh machine and we need to install some dependencies. However, some packages, such as the Tidyverse, are pre-installed. 
# First install system dependencies of `sf`:
...
system.time(BiocManager::install("Voyager", dependencies = TRUE, Ncpus = 2, update = FALSE))
