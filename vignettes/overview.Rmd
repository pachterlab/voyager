...
```{r, fig.alt="Plot of total UMI counts per Visium spot (nCounts) in physical space, with tissue boundary in the background. Total counts are high near the top and bottom of the tissue and low in the center at the injury site."}
plotSpatialFeature(sfe, features = "nCounts", colGeometryName = "spotPoly",
                   annotGeometryName = "tissueBoundary")
```

```{r, fig.alt="Plot of number of genes detected per Visium spot (nGenes) in physical space. Number of genes is higher in the regions surrounding but not within the injury site."}
plotSpatialFeature(sfe, features = "nGenes", colGeometryName = "spotPoly",
                   annotGeometryName = "tissueBoundary")
```

```{r, fig.alt="Plot of proportion of mitochondrially encoded UMI counts per Visium spot in physical space. Mitochondrial proportion is high far outside the injury site in tissue where myofibers are tightly packed. Mitochondrial proportion is also high outside tissue."}
...
```{r, fig.width=8, fig.height=3, fig.alt="The previous 3 plots shown side by side in one row."}
...
```{r, fig.alt="Scatter plot with nGenes in the y axis and nCounts in the x axis. The y axis ranges from 0 to 5000, and the x axis ranges from 0 to 20,000. The points represent Visium spots, and are concentrated near the origin and form two branches to the upper right away from the origin. The points are colored by whether they are in tissue. Those not in tissue are generally close to the origin, while the two branches are predominantly in tissue."}
...
```{r, fig.alt="Scatter plot with mitochondrial proportion (prop_mito) in the y axis and nCounts in the x axis. The x axis ranges from 0 to 20,000, and the y axis ranges from 0 to 0.7. Points represent Visium spots, and are colored by whether they are in tissue. Those not in tissue concentrate around a straight line where x = 0 and range from 0 to around 0.6 in y. Points in tissue form two clusters, one with nCounts ranging from around 0 to around 10,000 and prop_mito randing from around 0.05 to 0.25, and another with nCounts ranging from around 2000 to 20,000 and prop_mito from around 0.25 to 0.5."}
plotColData(sfe, x = "nCounts", y = "prop_mito", colour_by = "in_tissue")
```

```{r, fig.alt="3 panels of violin plots with logical indicator of whether a Visium spot is in tissue in the x axis and different y axis per panel. First panel: nCounts is vastly higher in tissue (mode around 2500, with long tail going above 20,000) than outside (almost all near 0). Second panel: nGenes is vastly higher in tissue (mode around 1500 with long tail going to 5000) than outside. Third panel: On average, prop_mito is comparable in and out of tissue, but in tissue, the distribution of prop_mito is bimodal, with one peak at around 0.15 and another around 0.35, and long tail going to 0.7."}
...
```{r, fig.width=8, fig.height=4, fig.alt="3 panels of plots of Visium spots in tissue in physical space, colored by: number of myofibers intersecting each spot (higher in the lower left and lower in the injury site), number of nuclei intersecting each spot (higher in the injury site), and proportion of area of each spot occupied by myofibers (1 or nearly 1 except around the injury site and tissue margins)."}
...
```{r, fig.alt="Scatter plot with nGenes in the y axis and nCounts in the x axis. Each point represents a Visium spot in tissue, colored by number of nuclei."}
plotColData(sfe_tissue, x = "nCounts", y = "nGenes", colour_by = "n_nuclei")
```

Mean myofiber area, number of myofibers per spot, and number of nuclei per spot seem to be related to the two branches in the nGenes vs. nCounts plot. The upper branch includes the inflamed injury site with lots of immune cells and hence nuclei and devoid of myofibers and the adjacent region with larger myofibers.

How does total UMI count relate to the number of nuclei per spot?
```{r, fig.alt="Scatter plot with number of nuclei in the y axis and nCounts in the x axis. Each point represents a Visium spot in tissue, colored by number of myofibers."}
...
```{r, fig.alt="Scatter plot with number of nuclei in the y axis and nCounts in the x axis. Each point represents a Visium spot in tissue, colored by proportion of area of each spot intersecting tissue."}
plotColData(sfe_tissue, x = "nCounts", y = "n_nuclei", 
            colour_by = "prop_tissue")
```

There might not be a clear relationship between the number of nuclei and total counts. There are some spots with both low total counts and small number of nuclei; those are at the margin of the tissue and don't fully cover tissue. There are also some spots with relatively low total counts and large number of nuclei.

How do the number and proportion area of myofibers per spot relate to the number of nuclei per spot?

```{r, fig.alt="Scatter plot with number of nuclei in the y axis and number of myofibers intersecting each Visium spot in the x axis. Each point represents a Visium spot in tissue, colored by proportion of area of each spot occupied by myofibers."}
