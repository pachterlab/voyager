...
sfe_tissue <- colDataMoransI(sfe_tissue, features = c("nCounts", "nGenes"),
                             colGraphName = "visium")
head(attr(colData(sfe_tissue), "featureData"), 10)
```

For `colData`, the results are added to the attribute called "featureData", and features for which Moran's I is not calculated have NA. The column names of `featureData` distinguishes between different samples (there's only one sample in this dataset), and are parsed by plotting functions. 
