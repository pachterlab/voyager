...
plotSpatialFeature(sfe_tissue, features = "n_myofibers", 
                   colGeometryName = "spotPoly") + theme_void()
```
...
df <- data.frame(myh2 = logcounts(sfe_tissue)[rowData(sfe_tissue)$symbol == "Myh2",],
                 Ii = localResult(sfe_tissue, "localmoran", "Myh2")[,"Ii"])
...
plotLocalResult(sfe_tissue, "localG_perm", features = "Myh2", 
                ...
                colGeometryName = "spotPoly", divergent = TRUE,
                diverge_center = -log10(0.05)) + theme_void()
```
...
For this gene, the local conditions are mostly homogenous, except for a few spots. Warm color indicates adjusted $p < 0.05$.
