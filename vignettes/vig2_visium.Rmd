...
This vignette gives an overview of some ESDA methods, functionalities of the `Voyager` package, and applications of the `SpatialFeatureExperiment` class with a published Visium dataset.
...
In the [Basic Visium vignette](https://pachterlab.github.io/Voyager/articles/vig2_visium_basic.html), we encountered the mysterious two branches and two clusters in the nGenes vs. nCounts plot and the proportion of mitochondrial counts vs. nCounts plot. The two clusters seem to be related to myofiber size.
```{r}
plotColData(sfe_tissue, x = "nCounts", y = "nGenes", color_by = "mean_myofiber_area")
```

```{r}
plotColData(sfe_tissue, x = "nCounts", y = "prop_mito", color_by = "mean_myofiber_area")
```
...
A spatial neighborhood graph is required to compute spatial dependency metrics such as Moran's I and Geary's C. The `SpatialFeatureExperiment` package wraps methods in `spdep` to find spatial neighborhood graphs, which are stored within the SFE object (see `spdep` documentation for `gabrielneigh()`, `knearneigh()`, `poly2nb()`, and `tri2nb()`). The `Voyager` package then uses these graphs for spatial dependency analyses, again based on `spdep` in this first version, but methods from other geospatial packages, some of which also use the spatial neighborhood graphs, may be added later as needed.
...
## Local Moran's I
To recap, global Moran's I [@Anselin1995-cs] is defined as
...
Plot the log normalized Myh2 gene expression as context to interpret the local results
```{r}
...
It seems that regions with higher Myh2 expression also have stronger spatial autocorrelation. It would be interesting to see how spatial autocorrelation relates to gene expression level, much as finding how variance relates to mean in the expression of each gene, which usually indicates overdispersion compared to Poisson in scRNA-seq and Visium data.

```{r}
qplot(logcounts(sfe_tissue)[rowData(sfe_tissue)$symbol == "Myh2",],
      localResult(sfe_tissue, "localmoran", "Myh2")[,"Ii"],
      xlab = "Myh2 (log counts)", ylab = "localmoran", alpha = I(0.3))
```
...
The hotspots and coldspots are as expected.
...
```{r}
localResultAttrs(sfe_tissue, "LOSH.cs", "Myh2")
```

While `Voyager` does wrap `LOSH.mc()` to perform permutation testing of LOSH, this is very time consuming. A Chi-square approximation is described in the 2012 LOSH paper to account for non-normality of the data and to approximate the mean and variance of the permutation distributions, so p-values of LOSH can be more quickly computed, with `LOSH.cs()`.
...
For this gene, the local conditions are mostly homogenous, with a few exceptions around the edge of the tissue.
...
2. The plotting functions don't plot the H&E image in the background. This is not implemented in the first version since when the Visium spots are plotted, they block out the H&E image anyway. Also, the plots are small and not interactive, which is good enough for visualizing gene expression on Visium spots, while the H&E image would be more informative when more zoomed in. Interactive data visualization is beyond the scope of this package.
...
4. Only 2D data is supported at present, although in principle, `sf` and `GEOS` support 3D data.
