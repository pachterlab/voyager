...
To examine the speed of Annoy, we convert the results into an `nb` object in `spdep`, and do inverse distance weighting. From profiling, most of the time constructed the spatial weights matrix was spent on `nb2listwdist()`, to perform distance weighting. The output from `findKNN()` contains a matrix for the indices of the neighbors, and another matrix for the distances to the neighbors. To further speed up the code, we directly compute the inverse distance edge weights without using `nb2listwdist()` and format the data into the `listw` structure. The functions `knn2nb()` and `nb2listwdist()` involve more slow R loops but are more flexible for many other kinds of graphs.
...
There's some spatial structure in PC2 and PC4 at a smaller scale.
