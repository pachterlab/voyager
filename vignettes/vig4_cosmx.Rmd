...
It took about a minute, not bad. Now compute Moran's I for some cell QC metrics
```{r}
...
These warnings only occur in `ShowParam()`, but not in `MulticoreParam()`, and they don't seem to affect the results. `ShowParam()` is used here since `MulticoreParam()` doesn't work on Windows, although this vignette itself was built on Linux. We did not notice significant performance differences between `ShowParam()` and `MulticoreParam()` in this context.
...
```{r}
# Note: on your computer, you can put progressbar = TRUE inside SnowParam()
