...
The data is binned by distance between spots and the semivariance is computed for each bin. The numbers by the points in the plot indicate the number of pairs of spots in each bin. "Ste" means the Matern model with M. Stein's parameterization was fitted to the points. 
...
Spatial autocorrelation makes the variance smaller at shorter distances. When the variogram levels off, it means that spatial autocorrelation no longer has an effect at this distance. Sill is the semivariance where the variogram levels off. Range is the distance where the variogram levels off. 
