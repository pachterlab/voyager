...
test_that("Correct structure of mc.sim output", {
  out <- calculateUnivariate(mat1, listw = colGraph(sfe, "visium", sample_id = "sample01"),
...
test_that("Correct structure of colDataUnivariate output, with list column", {
  out <- colDataUnivariate(sfe, type = "moran.mc", colGraphName = "visium",
...
test_that("Correct structure of colGeometryUnivariate output, with list column", {
  out <- colGeometryUnivariate(sfe, colGeometryName = "spotPoly",
                               type = "moran.mc",
                               colGraphName = "visium", features = "foo",
...
test_that("MoranMC results properly added to rowData", {
  sfe2 <- runUnivariate(sfe, type = "moran.mc", colGraphName = "visium",
...
test_that("DataFrame results for sp.correlogram", {
  out <- calculateUnivariate(mat1, type = "sp.correlogram",
...
names_expect_gg <- c("statistic", "p.value", "alternative", "data.name", "method",
...
names_expect_lm <- c("Ii", "E.Ii", "Var.Ii", "Z.Ii", "Pr(z != E(Ii))", "mean",
                     "median", "pysal")
...
names_expect_lg <- c("localG", "Gi", "E.Gi", "Var.Gi", "Pr(z != E(Gi))",
